...
import { serviceCategories, servicesFlat } from "@/lib/services-data"
...
export function generateMetadata({ params }: ServicePageProps): Metadata {
  const service = servicesFlat.find(
    (item) => item.slug === params.service && item.categoryId === params.category,
  )
    ...
    <main className="bg-white">
      <section className="relative overflow-hidden bg-slate-950">
        <div
          ...
          style={{
            backgroundImage: `linear-gradient(135deg, rgba(15, 23, 42, 0.85), rgba(30, 64, 175, 0.75)), url(${detail.heroImage})`,
        ...
        <div className="relative container mx-auto px-4 py-28">
          ...
          <div className="mt-10 max-w-3xl">
            ...
            <h1 className="mt-4 text-4xl font-bold leading-tight text-white md:text-5xl">{service.name}</h1>
            <p className="mt-4 text-lg text-white/85">{detail.heroSubtitle}</p>
            <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:items-center">
              <Button
                className="h-12 rounded-full bg-white px-6 text-base font-semibold text-[#1e3a8a] hover:bg-white/90"
                asChild
              >
          ...
          <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_380px]">
            <div className="space-y-6">
              {detail.description.map((paragraph) => (
                <p key={paragraph.slice(0, 24)} className="text-lg leading-relaxed text-gray-700">
                  {paragraph}
                </p>
              ))}

              <div>
                <h2 className="text-2xl font-semibold text-[#1e3a8a]">O que este serviço entrega</h2>
            ...
            <div className="rounded-3xl border border-slate-200 bg-slate-50/70 p-8 shadow-sm">
              ...
              <p className="mt-2 text-sm text-gray-600">
                Garantimos equipes alinhadas à cultura da sua empresa, com supervisão próxima e indicadores claros.
              </p>
              ...
              <div className="mt-8 rounded-2xl bg-white p-6 shadow-inner">
                ...
                <Button
                  asChild
                  className="mt-4 h-10 rounded-full bg-[#2563eb] text-white hover:bg-[#1e40af]"
                >
