...
import { GESTOR_SESSION_STORAGE_KEY } from "@/lib/constants"
...
type RhUser = { id: string; name: string; unidade: string; status: "ativo" | "pendente" }
...
const defaultRhUsers: RhUser[] = [
  { id: "rh-1", name: "Camila Duarte", unidade: "PE", status: "ativo" },
  { id: "rh-2", name: "Roberto Fernandes", unidade: "SP", status: "pendente" },
]
...
const defaultCandidates: Candidate[] = [
  ...
  { id: "JSP-2287", name: "Juliana Souza", area: "Saúde", status: "Entrevista", updatedAt: "16/04/2024" },
]
...
export default function GestorPage() {
  ...
  const [newRhUser, setNewRhUser] = useState({ name: "", unidade: "" })
...
const stats = useMemo(
  ...
  const handleAddRhUser = () => {
    if (!newRhUser.name || !newRhUser.unidade) {
      return
    }
    setRhUsers((prev) => [
      ...prev,
      { id: `rh-${Date.now()}`, name: newRhUser.name, unidade: newRhUser.unidade.toUpperCase(), status: "pendente" },
    ])
    setNewRhUser({ name: "", unidade: "" })
  }
      ...
      <div className="mx-auto max-w-6xl px-4">
        ...
        {activeTab === "clients" && (
          <section className="mt-8 rounded-3xl bg-white p-6 shadow">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {clients.map((client) => (
                <article key={client.id} className="overflow-hidden rounded-3xl border border-gray-100 shadow-sm">
                  <img src={client.coverImage} alt={client.name} className="h-48 w-full object-cover" />
                  <div className="p-6 space-y-3">
                    <div className="flex items-start justify-between">
                      ...
                      <button
                        ...
                        className="rounded-full border border-gray-200 p-2 text-gray-500 transition hover:text-[#ef4444]"
                      >
                    ...
                    <button className="w-full rounded-full border border-[#1e3a8a] px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-[#1e3a8a] hover:bg-[#eff6ff]">
                      ver case completo →
                    </button>
                  </div>
                </article>
            ...
            <div className="mt-8 space-y-4 rounded-3xl border border-dashed border-gray-200 p-6">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <input
                  ...
                  className="rounded-2xl border border-gray-200 px-4 py-3 text-sm outline-none transition focus:border-[#2563eb]"
                />
                <input
                  type="text"
              ...
              <textarea
                placeholder="Descrição breve sobre o projeto"
              ...
              <input
                type="text"
                placeholder="Tags separadas por vírgula (ex: Administrativo, Limpeza)"
        ...
        {activeTab === "rh" && (
          <section className="mt-8 rounded-3xl bg-white p-6 shadow">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.3em] text-[#2563eb]">Usuários do RH</p>
              ...
              {rhUsers.map((user) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-gray-500">Unidade {user.unidade}</p>
                  </div>
                  <button
                    type="button"
            ...
            <div className="mt-6 grid gap-3 md:grid-cols-2">
              ...
              <input
                type="text"
                placeholder="Unidade (UF)"
                value={newRhUser.unidade}
                onChange={(event) => setNewRhUser((prev) => ({ ...prev, unidade: event.target.value }))}
                className="rounded-2xl border border-gray-200 px-4 py-3 text-sm outline-none transition focus:border-[#2563eb]"
              />
            </div>
            <button
              ...
              className="mt-4 inline-flex items-center rounded-full bg-[#2563eb] px-5 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-white shadow transition hover:bg-[#1d4ed8]"
            >
              <Plus className="mr-2 h-4 w-4" />
              adicionar usuário
            </button>
